...
import { useGetAvailableDrivers } from "@/hooks/rides/getAvailableOperators";
...
const ReassignDriver = ({ isOpen, onClose, rideData }: ReassignDriverProps) => {
  const { mutate: reassignRides, isPending } = useReassignRide(
    rideData?.id ?? "",
  );

  const { data: availableDriver, isLoading: isDriversLoading } =
    useGetAvailableDrivers(rideData?.id ?? "");
    ...
    >
      <div className="flex flex-col gap-7">
        ...
        <Select
          ...
          placeholder={
            isDriversLoading ? "Loading drivers..." : "Select a driver"
          }
          register={registerReassignDriver}
