...
const ScheduledCargoList: React.FC<AllTripsProps> = ({
  allTrips,
  currentPage,
  totalItems,
  itemsPerPage,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: TripsDetailsResponse) => <p>{row?.payment_method}</p>,
    },
    {
    ...
    },
    {
      ...
      accessor: "scheduled_at" as keyof TripsDetailsResponse,
      render: (row: TripsDetailsResponse) => (
        <span>{formatDate(Number(row?.scheduled_at?.seconds) * 1000)}</span>
      ),
    },
    {
      header: "Actions",
...
export default ScheduledCargoList;
