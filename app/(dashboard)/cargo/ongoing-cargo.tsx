...
const OngoingCargoList: React.FC<AllTripsProps> = ({
  allTrips,
  currentPage,
  totalItems,
  itemsPerPage,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: TripsDetailsResponse) => (
        <div className="flex gap-2 items-center">
          <Avatar>
            <AvatarImage src={undefined} alt={row.customer?.full_name ?? ""} />
            <AvatarFallback>
  ...
  return (
    <div>
      <Table //@ts-expect-error will work on it
        columns={columns}
        data={allTrips}
        isLoading={isLoading}
      />
...
export default OngoingCargoList;
