...
import { useGetTowTripList } from "@/hooks/tow/getTripsList";
import { useGetTowTripStats } from "@/hooks/tow/getTripsStats";
...
const TowTripDetails: React.FC<TripProps> = ({ id }) => {
  const itemsPerPage = 20;
  const [currentPage, setCurrentPage] = useState<number>(1);
  const { data: tripStatData } = useGetTowTripStats(id);
  const { data: tripList, isLoading: tripLoading } = useGetTowTripList(id, {
    page: currentPage,
    limit: itemsPerPage,
  });
