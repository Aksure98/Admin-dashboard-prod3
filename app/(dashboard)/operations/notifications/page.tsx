...
import { useGetTowNotification } from "@/hooks/tow/getTowNotification";
import { ComposeNotificationsProps, DriverNotificationItem } from "@/@types";
import { useAddTowNotification } from "@/hooks/tow/addNotification";

const TowNotifications = () => {
  ...
  const { mutateAsync: sendNotification, isPending: isSending } =
    useAddTowNotification();

  const { data: notificationData } = useGetTowNotification({
    page: currentPage,
  ...
  const filteredData = useMemo(() => {
    ...
    if (!term) return allNotifications;
    return allNotifications.filter((n) => {
      return (
        n.title.toLowerCase().includes(term) ||
        n.body.toLowerCase().includes(term) ||
        n.recipients.toLowerCase().includes(term)
      );
    });
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: DriverNotificationItem) => (
        <span className="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-success-100 text-success-600">
          {row.status}
        </span>
      ),
    },
    {
      header: "Actions",
      accessor: "id" as keyof DriverNotificationItem,
      sortable: false,
      render: (row: DriverNotificationItem) => (
        <div className="flex items-center gap-4 text-sm font-bold">
          ...
          <button
            ...
            onClick={() => console.log("Delete notification", row?.id)}
          >
            Remove
          </button>
        </div>
  ...
  return (
    <div className="flex flex-col gap-6 mt-24">
      ...
      <div className="border border-grey-200 rounded-lg shadow">
        ...
        <Table
          columns={columns}
          data={filteredData.length ? paginatedData : notificationList ?? []}
          selectable
      ...
      <ComposeNotifications
        ...
        // @ts-expect-error same as riders flow
        onSend={handleSend}
        isSending={isSending}
      />
        ...
        >
          <div className="flex flex-col gap-6">
            ...
            <div>
              ...
              <span className="mt-2 px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-success-100 text-success-600 w-fit">
...
export default TowNotifications;
