...
import { useDriversWallet } from "@/hooks/drivers/getDriverWallet";
import { keyof } from "zod";
import { WalletItem } from "@/@types";
import Link from "next/link";
import { useExportDriversWallet } from "@/hooks/drivers/exportDriversWallet";
...
const DriversWallet = () => {
  ...
  const exportMutation = useExportDriversWallet("RIDES");

  const { data: walletData, isLoading } = useDriversWallet({
    page: currentPage,
  ...
  const columns = [
    ...
    {
      header: "Service Type",
      accessor: "service_type" as keyof WalletItem,
    },
    {
...

    {
      ...
      render: (row: WalletItem) => (
        <Link
          href={`/operators/wallet/${row?.user_id}`}
    ...
    <div className="flex flex-col gap-6 mt-24">
      ...
      <div className="grid grid-cols-3 gap-5">
        <div className="border border-grey-200 rounded-3xl p-6">
          ...
          <p className="text-grey-800 text-4xl font-bold">
            {formatPrice(walletDetails?.total_balance ?? 0)}
          </p>
        </div>
        <div className="border border-grey-200 rounded-3xl p-6">
          <h3 className="text-grey-600 text-sm font-bold font-figtree">
          ...
          <p className="text-grey-800 text-4xl font-bold">
            {formatPrice(walletDetails?.pending ?? 0)}
          </p>
        </div>
        <div className="border border-grey-200 rounded-3xl p-6">
          <h3 className="text-grey-600 text-sm font-bold font-figtree">
          ...
          <p className="text-grey-800 text-4xl font-bold">
            {formatPrice(walletDetails?.withdraws ?? 0)}
          </p>
        </div>
      </div>
      ...
      <div className="border border-grey-200 rounded-lg shadow">
        ...
        <Pagination
          ...
          totalItems={walletData?.data?.total ?? 0}
          itemsPerPage={walletData?.data?.page_size ?? 20}
