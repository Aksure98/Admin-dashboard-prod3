...
type SuspendDriverProps = z.infer<typeof SuspendDriverSchema>;
...
interface SuspendDriverFormProps {
  isOpen: boolean;
  onClose: () => void;
...
const SuspendDriver = ({
  ...
  onConfirm,
  isLoading = false,
}: SuspendDriverFormProps) => {
  const {
    register: formRegister,
    handleSubmit,
    formState: { errors },
    reset,
  } = useForm<SuspendDriverProps>({
    resolver: zodResolver(SuspendDriverSchema),
  });

  const handleSuspendAccount = async (data: SuspendDriverProps) => {
    ...
    // Let the parent handle closing — don't call onClose() here
    // so customerToSuspend is still available when onConfirm fires
    if (onConfirm) {
