...
import { useGetEarning } from "@/hooks/drivers/getEarning";
import { useGetEarningList } from "@/hooks/drivers/earningList";

interface EarningProps {
  id: string;
}

const DriverEarnings: React.FC<EarningProps> = ({ id }) => {
  const itemsPerPage = 10;
  const [currentPage, setCurrentPage] = useState<number>(1);
  const { data: earningData, isLoading } = useGetEarning(id);
  const { data: earningList, isLoading: earningLoading } = useGetEarningList(
    id,
    {
      page: currentPage,
      limit: itemsPerPage,
  ...
  const handleViewTransaction = (transaction: DriverTransaction) => {
  ...
  const columns = [
    {
      header: "Id",
      accessor: "job_id" as keyof EarningItem,
    },
    {
      header: "Type",
...

    {
      ...
      render: (row: EarningItem) => <p>{formatDate(row?.created_at)}</p>,
    },
    // {
    //   header: "Actions",
    //   accessor: "" as keyof EarningItem,
    //   sortable: false,
    //   render: (row: EarningItem) => (
    //     <div
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="border border-grey-200 rounded-lg shadow">
        ...
        {/* <Table columns={column} data={allBank} />
        <Pagination
          currentPage={currentPage}
          totalPages={totalPages}
          totalItems={allBank.length}
