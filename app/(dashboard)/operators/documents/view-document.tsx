...
interface ViewDriverDocumentProps {
  isOpen: boolean;
  onClose: () => void;
...
const ViewDriverDocument: React.FC<ViewDriverDocumentProps> = ({
  isOpen,
  onClose,
  document,
  onApprove,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-end bg-black/40">
      <div className="relative bg-grey-0 w-full max-w-xl overflow-y-auto scrollbar-hide flex flex-col rounded-l-3xl shadow-xl h-fit">
        ...
        {/* Footer buttons — hidden when APPROVED */}
        {!isApproved && !isRejected && (
          <div className="px-6 py-5 border-t border-grey-200 flex gap-3">
            <button
              onClick={onReject ?? onClose}
              disabled={isBusy}
              className="flex-1 px-4 py-2.5 border border-grey-300 rounded-lg text-grey-700 text-sm font-semibold hover:bg-grey-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              disabled={isBusy}
              className="flex-1 px-4 py-2.5 bg-brand-600 rounded-lg text-white text-sm font-semibold hover:bg-brand-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <PlusIcon size={16} />
...
export default ViewDriverDocument;
