...
import { useCargoWallet } from "@/hooks/cargo/getCargoWallet";
import { WalletItem } from "@/@types";
import Link from "next/link";
import { useExportCargoWallet } from "@/hooks/cargo/exportCargoWallet";
...
const CargoWallet = () => {
  ...
  const exportMutation = useExportCargoWallet();

  const { data: walletData, isLoading } = useCargoWallet();
  ...
  const columns = [
...

    {
      ...
      render: (row: WalletItem) => (
        <Link
          href={`/cargo-operator/wallet/${row?.wallet_id}`}
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-grey-800 text-2xl font-bold font-figtree">
            Cargo Drivers Wallet Management
          </h1>
        </div>
