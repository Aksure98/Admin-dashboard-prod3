...
import { useGetCargoLocationList } from "@/hooks/cargo/getCargoLocation";
...
const CargoLocation = () => {
  ...
  const { data: cargoLocation, isLoading } =
    useGetCargoLocationList(selectedStatus);
  const cargoList = cargoLocation?.data?.operators ?? [];

  const filteredDrivers = cargoList.filter((driver) => {
        ...
        <div className="w-full h-full rounded-lg overflow-hidden border border-grey-200 bg-grey-0 flex items-center justify-center">
          {mapSrc ? (
            ...
            <p className="text-grey-400 text-sm">
              Click &quot;View Location&quot; to see a cargo driver on the map
            </p>
          )}
        </div>
      </div>
