...
import { useCargoDocument } from "@/hooks/cargo/getCargoDocument";
import { DocumentItem } from "@/@types";
import { useRejectCargoDocument } from "@/hooks/cargo/rejectCargoDoc";
import { useApproveCargoDocument } from "@/hooks/cargo/approveCargoDoc";
...
export default function CargoDocumentsPage() {
  ...
  const { mutate: rejectDoc } = useRejectCargoDocument(
    selectedDocument?.user_id ?? "",
    selectedDocument?.id ?? "",
  );
  const { mutate: approveDoc } = useApproveCargoDocument(
    selectedDocument?.user_id ?? "",
    selectedDocument?.id ?? "",
  );

  const { data: documentData, isLoading } = useCargoDocument();
