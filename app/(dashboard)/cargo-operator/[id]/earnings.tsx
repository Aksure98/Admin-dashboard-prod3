...
import { useGetCargoEarning } from "@/hooks/cargo/getEarning";
import { useGetCargoEarningList } from "@/hooks/cargo/earningList";

interface EarningProps {
  id: string;
}

const CargoEarnings: React.FC<EarningProps> = ({ id }) => {
  const itemsPerPage = 10;
  const [currentPage, setCurrentPage] = useState<number>(1);
  const { data: earningData } = useGetCargoEarning(id);
  const { data: earningList, isLoading: earningLoading } =
    useGetCargoEarningList(id, {
      page: currentPage,
      limit: itemsPerPage,
    });
