...
import { useUpdateBaseFare } from "@/hooks/pricing/updateBaseFare";

const EditBaseFareSchema = z.object({
  amount: z.number({ message: "Base fare amount is required" }).min(0, {
...
const EditBaseFare = ({
  ...
  const editBaseFareMutation = useUpdateBaseFare();
  ...
  const handleSave = async (data: EditBaseFareProps) => {
    await editBaseFareMutation.mutateAsync(
      { id: baseFareId, amount: data.amount },
      {
        onSuccess: () => {
          onClose();
        },
