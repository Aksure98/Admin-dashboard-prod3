...
import { useUpdateDriverCommission } from "@/hooks/pricing/updateDriverCommission";
...
const EditDriverCommission = ({
  isOpen,
  onClose,
  distanceId,
  currentAmount,
}: EditBaseFareFormProps) => {
  const editBaseFareMutation = useUpdateDriverCommission();
  ...
  const handleSave = async (data: EditBaseFareProps) => {
    await editBaseFareMutation.mutateAsync(
      { id: distanceId, percentage: data.amount },
      {
        onSuccess: () => {
          onClose();
        },
  ...
  return (
    <FormModal
      title="Edit Driver's commission"
      saveButtonText="Update percentage"
...
export default EditDriverCommission;
