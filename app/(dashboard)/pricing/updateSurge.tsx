...
import { useUpdateSurge } from "@/hooks/pricing/updateSurge";
...
const EditSurge = ({
  ...
  const editBaseFareMutation = useUpdateSurge();
  ...
  const handleSave = async (data: EditBaseFareProps) => {
    await editBaseFareMutation.mutateAsync(
      { id: distanceId, multiplier: data.per_km },
      {
        onSuccess: () => {
          onClose();
        },
