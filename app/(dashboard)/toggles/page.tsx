...
import { useGetFeatures } from "@/hooks/features/getFeatures";
import { useToggleFeatureStatus } from "@/hooks/features/toggleFeatureStatus";
...
const FeatureTogglePage = () => {
  ...
  const featureList = (features?.data ?? []).map((item) => ({
    ...item,
    id: item.feature_id,
  }));
  ...
  const totalPages = Math.ceil(
    (features?.total ?? 0) / (featureList?.length ?? 20),
  );
  ...
  const handleToggleStatus = (row: FeatureToggle) => {
    setTogglingId(row.feature_id);
    toggleStatusMutation.mutate(
      { id: row.feature_id, value: { is_active: !row.is_active } },
  ...
  const columns = [
    {
      header: "Name",
      accessor: "name" as keyof FeatureToggle,
    },
    {
      header: "Category",
      accessor: "category" as keyof FeatureToggle,
    },
    {
      header: "Status",
      accessor: "is_active" as keyof FeatureToggle,
      render: (row: FeatureToggle) => (
        <span
    ...
    {
      header: "Actions",
      accessor: "id" as keyof FeatureToggle,
      sortable: false,
      render: (row: FeatureToggle) => (
        <div className="flex items-center space-x-2">
          <label
            className={`relative inline-flex items-center ${
              togglingId === row.feature_id
          ...
          >
            <input
              ...
              disabled={togglingId === row.feature_id}
    ...
    <div className="flex flex-col gap-6 mt-24">
      ...
      <div>
        <div className="border border-grey-200 rounded-lg shadow">
          ...
          <Table columns={columns} data={featureList} isLoading={isLoading} />

          <Pagination
            currentPage={currentPage}
            totalPages={totalPages}
            totalItems={features?.total || 0}
            itemsPerPage={featureList?.length || 20}
